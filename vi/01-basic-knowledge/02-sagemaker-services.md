...
> **Một câu:** "Đội quân dán nhãn" — thuê người thật khoanh vùng/gắn nhãn dữ liệu để dạy AI, có AI phụ làm việc dễ cho rẻ.
...
> **Một câu:** Làm sạch dữ liệu bằng **kéo-thả** (visual, gần như không code).
...
> **Một câu:** "Máy cày data" — viết code Python/Spark, ném cho nó tự bật cụm máy chủ cày sạch đống data khổng lồ rồi tự tắt.
...
> **Một câu:** "Tủ lạnh chung" của công ty chứa các **đặc trưng (feature)** đã tính sẵn, để mọi team lấy ra tái dùng thay vì tính lại từ đầu.
...
> **Một câu:** "Mái nhà chung" bao trùm mọi dịch vụ SageMaker — cung cấp máy chủ, thuật toán để train & host model. Đây là **tên mới** của Amazon SageMaker (đổi tại re:Invent **cuối 2024**).
...
> **Một câu:** "Siêu thị mô hình" — lấy sẵn Foundation Model mã nguồn mở (Llama, FLAN-T5, Stable Diffusion) thay vì xây "não" từ số 0.
...
> **Một câu:** "Băng chuyền" nối các bước ML thành quy trình tự động — **CI/CD cho Machine Learning (MLOps)**.
...
> **Một câu:** "Sổ hộ khẩu / GitHub cho model" — quản version, ai tạo, trạng thái, và phải **approve** mới được dùng.
...
> **Một câu:** "Bảo vệ gác cổng" — canh model ở Production, hú còi (qua CloudWatch) khi độ chính xác sụt giảm.
...
> **Một câu:** "Thanh tra công lý" — trả lời **vì sao** AI ra quyết định (explainability) và phát hiện **thiên vị** (bias).
...
> **Một câu:** "Thợ độ xe" — biên dịch & tối ưu model nặng để chạy mượt trên thiết bị edge/IoT/mobile nhỏ.
...
> **Một câu:** "Bàn làm việc đa năng" — gom Athena/SageMaker/Bedrock… vào một màn hình, kèm AI (Amazon Q) ngồi cạnh viết code hộ.
